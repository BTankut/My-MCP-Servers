...
# Create wrapper script for custom server
create_wrapper_script() {
    local SERVER=$1
    local SERVER_PATH=$2
    local SERVER_CONFIG=$3
    
    local WRAPPER_PATH="$(dirname "$SERVER_PATH")/${SERVER}-wrapper.sh"
    
    print_info "Creating wrapper script: $WRAPPER_PATH"
    
    # Get environment variables from config
    local ENV_VARS=($(echo "$SERVER_CONFIG" | jq -r '.api_key_config.env_vars[]?.name // empty'))
...
EOF
    ...
    for var in "${ENV_VARS[@]}"; do
        if [ -n "${!var}" ]; then
            echo "export $var=\"${!var}\"" >> "$WRAPPER_PATH"
            print_success "Added $var to wrapper"
        else
            echo "# export $var=\"your_${var,,}_here\" # TODO: Set this value" >> "$WRAPPER_PATH"
            print_warning "$var not set - added as comment"
        fi
    done
    
    # Add server execution line
    cat >> "$WRAPPER_PATH" << EOF
...
install_custom_server() {
    ...
    if [ "$REQUIRES_KEY" = "true" ]; then
        print_info "Creating wrapper script for API key handling..."
        WRAPPER_PATH=$(create_wrapper_script "$SERVER" "$SERVER_PATH" "$SERVER_CONFIG")
        
        # Use wrapper instead of direct path
        FULL_COMMAND="claude mcp add \"$SERVER\" \"$WRAPPER_PATH\" --scope user"
    else
        # Direct installation without wrapper
    ...
    if eval "$FULL_COMMAND" 2>/dev/null; then
        ...
        if [ "$REQUIRES_KEY" = "true" ]; then
            print_info "Using wrapper script for environment variables"
            print_warning "Edit wrapper if needed: $WRAPPER_PATH"
        fi
        return 0
    else
